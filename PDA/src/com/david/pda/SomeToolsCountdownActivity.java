...
public class SomeToolsCountdownActivity extends Activity {
	...
	private void resolveIntent() {
		...
		if (from != null) {
			if (from.equals(SomeToolsCountdownOptionActivity.class.getName())) {
				if (intent.getFlags() == SomeToolsCountdownOptionActivity.ADD_SUCCESS) {
					Toast.makeText(SomeToolsCountdownActivity.this, "填加成功！",
							Toast.LENGTH_SHORT).show();
				} else if (intent.getFlags() == SomeToolsCountdownOptionActivity.UPDATE_SUCCESS) {
					Toast.makeText(SomeToolsCountdownActivity.this, "更新成功！",
							Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(SomeToolsCountdownActivity.this, "操作失败！",
							Toast.LENGTH_SHORT).show();
				}
			}
		}
		...
		public boolean onItemLongClick(AdapterView<?> arg0, View arg1,
			...
			try {
				...
				Toast.makeText(SomeToolsCountdownActivity.this,
						"删除《" + countdownList.get(index).getTitle() + "》成功！",
						Toast.LENGTH_SHORT).show();
				countdownList.remove(index);
				initGrid();
			} catch (JSONException e) {
				e.printStackTrace();
				Toast.makeText(SomeToolsCountdownActivity.this, "删除失败！",
						Toast.LENGTH_SHORT).show();
			}
			return false;
		}
